...
def get_lowest_next_number(current_play: int = Field(description="""current play"""), selected_number: int = Field(description="""selected number""")):

    """
    Your task is to select a number from a play list that consists of a series of numbers where the possible values in increasing order are [3, 4, 5, 6, 7, 8, 9, 11, 12, 13, 14].
    Select the lowest number in the play list that is greater than or equal to the current play.
    ...
    After selecting, run a sanity check to ensure that the selected number is actually equal or greater than the current play.
    If the sanity check fails you must select a new number.

    Select 0 if there are no numbers in the play list that are greater than or equal to the current play. 
    ...
    If the current play is 7 and the numbers from the play list are [5] the selected number is 0 since no numbers in the play are greater than or equal to the current play.
    ...
    If the current play is 9 and the numbers from the play list are [5, 4, 3] the correct number is 0 since no numbers in the play are greater than or equal to the current play.
